defmodule Pair.Recordings.Workers.InsightsWorkerTest do
  ...
  describe "perform/1" do
    ...
    test "handles meeting notes extraction errors gracefully" do
      ...
      assert {:error, "Failed to extract structured notes"} =
               perform_job(InsightsWorker, job_args)
    ...
    test "handles database update errors" do
      ...
      job_args = %{"id" => recording.id}
      assert {:error, %Ecto.Changeset{}} = perform_job(InsightsWorker, job_args)
    end
  end
  ...
  describe "enqueue/1" do
    test "creates actions job with correct arguments" do
      ...
      assert %Oban.Job{} = job = InsightsWorker.enqueue(recording)
    ...
    test "job is properly inserted into the database" do
      ...
      assert {:ok, job} = InsightsWorker.enqueue(recording)
  ...
  describe "integration test" do
    test "full actions workflow with both legacy and structured outputs" do
      ...
      job = InsightsWorker.enqueue(recording)
      assert :ok = perform_job(InsightsWorker, job.args)
